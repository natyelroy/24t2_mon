...
void print_final_order(struct bubble_tea order, double total_cost);
...
int main(void) {
    ...
    print_final_order(order, total_cost);
...
// this updates the stock of inventory, including tea and toppings
struct inventory update_stock(struct bubble_tea order, struct inventory inventory) {
    if (order.size == LARGE) {
        inventory.tea -= 2;
    } else {
